...
export const checkBackendConnection = async () => {
  ...
  try {
    ...
    if (healthResponse.ok) {
      ...
      results.health_check = { success: true, data: healthData };
      console.log('✅ [DEBUG] Health check passed:', healthData);
    } else {
      results.health_check = { success: false, status: healthResponse.status, statusText: healthResponse.statusText };
      console.log('❌ [DEBUG] Health check failed:', healthResponse.status, healthResponse.statusText);
    }
  } catch (error) {
    results.health_check = { success: false, error: error.message };
    console.log('❌ [DEBUG] Health check error:', error);
  }

  // 2. Test API
  try {
    console.log('🔍 [DEBUG] Testing /api/test...');
    ...
    if (testResponse.ok) {
      ...
      results.test_api = { success: true, data: testData };
      console.log('✅ [DEBUG] Test API passed:', testData);
    } else {
      results.test_api = { success: false, status: testResponse.status, statusText: testResponse.statusText };
      console.log('❌ [DEBUG] Test API failed:', testResponse.status, testResponse.statusText);
    }
  } catch (error) {
    results.test_api = { success: false, error: error.message };
    console.log('❌ [DEBUG] Test API error:', error);
  }

  // 3. Holidays API
  try {
    console.log('🔍 [DEBUG] Testing /api/holidays...');
    const holidaysResponse = await fetch(`${API_BASE_URL}/holidays`, {
      method: 'GET',
      mode: 'cors',
      credentials: 'omit'
    });
    ...
    if (holidaysResponse.ok) {
      ...
      results.holidays_api = { success: true, data: holidaysData };
      console.log('✅ [DEBUG] Holidays API passed:', holidaysData);
    } else {
      results.holidays_api = { success: false, status: holidaysResponse.status, statusText: holidaysResponse.statusText };
      console.log('❌ [DEBUG] Holidays API failed:', holidaysResponse.status, holidaysResponse.statusText);
    }
  } catch (error) {
    results.holidays_api = { success: false, error: error.message };
    console.log('❌ [DEBUG] Holidays API error:', error);
  }

  console.log('🔍 [DEBUG] Backend connection test results:', results);
  return results;
};

// 브라우저 콘솔에서 사용할 수 있는 전역 디버깅 함수
if (typeof window !== 'undefined') {
  window.debugBackend = checkBackendConnection;
  window.testAPI = () => {
